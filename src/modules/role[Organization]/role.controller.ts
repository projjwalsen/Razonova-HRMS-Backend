...
export const unassignRoleFromUser = async(req: Request, res: Response) => {
    try {
        ...
        /* Unassign role from user logic here */
        if(!user?.tenantId || user.tenantId !== targetUser.tenantId){
            return res.status(403).json({
                success: false,
                message: "Unauthorized to perform this action on user from different organization"
            })
        }
        const role = await prisma.role.findUnique({
            where: { id: roleId },
        ...
        if(targetUser.tenantId !== user.tenantId || role.tenantId !== user.tenantId || role.type !== "TENANT"){
            return res.status(403).json({
                success: false,
                message: "Unauthorized to perform this action on role from different organization or system role"
            })
        }
        ...
        // Protect the COMPANY_ADMIN
        if(role.name === "COMPANY_ADMIN"){
            const companyAdminCount = await prisma.userRole.count({
                where: { roleId }
            });
            if(companyAdminCount <= 1){
                return res.status(400).json({
                    success: false,
                    message: "Cannot unassign the last COMPANY_ADMIN from the organization"
                })
            }
        }
